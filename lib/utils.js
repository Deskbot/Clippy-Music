const fs = require('fs');
const crc_32 = require('crc-32');

module.exports = {
	arrShuffle: function arrShuffle(arr) {
		let tmpItem, randIndex;

		for (let i = 0; i < arr.length; i++) {
			randIndex = Math.floor(Math.random() * arr.length);
			
			tmpItem = arr[i];
			arr[i] = arr[randIndex];
			arr[randIndex] = tmpItem;
		}

		return arr;
	},

	clone: function clone(o) {
		if (null == o || "object" != typeof o) return o;
		var copy = o.constructor();
		for (var attr in o) {
			if (o.hasOwnProperty(attr)) copy[attr] = o[attr];
		}
		return copy;
	},

	deleteFile: function deleteFile(path) {
		fs.unlink(path, (err) => {
			if (err) {
				console.error('Attempt to delete file failed: ' + path + ' Error message:');
				console.error(err.message);
			}
		});
	},

	deleteFileIfExistsSync: function deleteFileIfExistsSync(dir) {
		try { fs.unlinkSync(dir); } catch(e) {}
	},

	deleteFolderRecursive: function deleteFolderRecursive(path) {
		if( fs.existsSync(path) ) {
			fs.readdirSync(path).forEach(function(file,index){
				var curPath = path + "/" + file;

				if(fs.lstatSync(curPath).isDirectory()) { // recurse
					deleteFolderRecursive(curPath);
				} else { // delete file
					fs.unlinkSync(curPath);
				}
			});
		}
	},

	extractYtVideoId: function extractYtVideoId(s) {
		let i = s.indexOf('?');
		s = s.substr(i);
		i = s.indexOf('?v=');
		if (i === -1) {
			i = s.indexOf('&v=');
			if (i === -1) return null;
		}
		s = s.substr(i + 3);
		i = s.indexOf('&');
		return s.substring(0, i === -1 ? s.length : i);
	},

	fileHash: function fileHash(path) {
		const file = fs.readFileSync(path);
		return crc_32.buf(file);
	},

	mkdirSafelySync: function mkdirSafelySync(path, mode) {
		try { fs.mkdirSync(path, mode); } catch(e) {}
	},

	randIntBetween: function randIntBetween(x,y) { //can include x but not y. Integers only
		return x + Math.floor(Math.random() * (y-x));
	},
	
	secToMinStr: secToMinStr = (s) => {
		const mins = Math.floor(s/60);
		const secs = s % 60;
		return `${mins}m${secs}s`;
	},

	secToTimeStr: secToTimeStr = (s) => {
		//const days = Math.floor(s / 86400);
		//const secsInDay = s % 86400;
		//const hours = Math.floor(secsInDay / 3600);

		const hours = Math.floor(s / 3600);
		const secsInHour = s % 3600;
		const mins = Math.floor(secsInHour / 60);
		const secs = s % 60;

		//let str = days > 0 ? days + 'd' : '';
		let str = hours > 0 ? ' ' + hours + 'h' : '';
		str +=    mins > 0  ? ' ' + mins  + 'm' : '';
		str +=    secs > 0  ? ' ' + secs  + 's' : '';
		return str.trim();
	},

	sizeToReadbleStr: function sizeToReadbleStr(s) {
		if (s > 1000000000) return '~' + Math.ceil(s / 1000000000) + 'GB';
		if (s > 1000000)    return '~' + Math.ceil(s / 1000000)    + 'MB';
		if (s > 1000)       return '~' + Math.ceil(s / 1000000)    + 'kB';
		return s + 'B';
	},

	//assume at most 2 colons
	timeCodeToNum: function timeCodeToNum(s) {
		const a = s.split(':');
		let t = 0;

		for (let i = 0; i < a.length; i++) {
			t += a[i] * Math.pow(60, a.length - 1 - i);
		}

		return t;
	},

	//altered version of https://stackoverflow.com/questions/10420352/converting-file-size-in-bytes-to-human-readable
	toShortSizeString: function toShortSizeString(fileSizeInBytes) {
		var i = -1;
		var byteUnits = ['k', 'm', ' g', ' t', 'p', 'e', 'z', 'y'];
		do {
			fileSizeInBytes = fileSizeInBytes / 1024;
			i++;
		} while (fileSizeInBytes > 1024);

		return (Math.max(fileSizeInBytes, 0.1).toFixed(1) + byteUnits[i]);
	},

	ytTimeStrToSec: function ytTimeStrToSec(str) {
		let timeArr = str.split(':');
		if (timeArr.length === 2)      return timeArr[0] * 60 + parseInt(timeArr[1]);
		else if (timeArr.length === 3) return timeArr[0] * 3600 + timeArr[1] * 60 + parseInt(timeArr[2]);
		else throw `Unable to convert yt time, ${str}, to seconds.`;
	},
};
