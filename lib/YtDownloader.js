...
class YtDownloader extends EventEmitter {
	constructor(progQueue) {
		super();
	...
	downloadNext(uid) {
		...
		const { deleter, updater } = this.progQueue.add(userId, title);
...
class PercentReader {
	...
	get() {
		...
		let pc = YtDownloader.extractPercent(data.toString());
