...
class YtDownloader {
	...
	downloadNext(uid) {
		...
		dlProm.then(() => {
			...
			if (head.cancelled) {
				defer.reject(new CancelError('Download was cancelled.', 'music'));
			} else {
				defer.reject(new DownloadError(e.message, 'music'));
			}
		});
	...
	new(cid, userId, title, vid, destination) {
		...
		defer.promise.then(() => {
			this.progressQueue.finished(userId, cid);
		}, () => {
			this.progressQueue.finishedWithError(userId, cid);
		});
