...
class ContentManager extends EventEmitter {
	...
	playNext() {
		...
		musicProc.on('close', (code, signal) => {
			let secs = Math.floor((Date.now() - contentData.timePlayedAt) / 1000); //seconds ran for
	...
	tryQueue(itemData) {
		...
		.then(() => {
			...
			this.remember(itemData); //save hashes
		...
		if (itemData.music.isUrl) {
			if (itemData.duration <= opt.streamYtOverDur) {
				...
				.then(() => {
					...
					let musicHash = utils.fileHash(nmp).toString();
			...
			return Promise.resolve().then(() => {
				//validate by music hash
				let musicHash = utils.fileHash(itemData.music.path).toString();
				if (this.hashIsUnique(musicHash)) {
					itemData.music.hash = musicHash;
				} else {
					throw new UniqueError('music');
	...
	tryPrepPicture(itemData) {
		...
		return promPic.then(() => {
			const picHash = utils.fileHash(itemData.pic.path);
		...
		return new Promise(function(resolve, reject) {
			...
			if (that.ytIdIsUnique(ytId)) {
				that.downloadYtInfo(uplData.music.path).then((info) => {
					...
					console.error(e);
					reject('Failed to retreive data about the YouTube video requested: ' + JSON.stringify(uplData));
				});
			
			} else {
				reject(`The YouTube video (${ytId}) requested has been played within the past ${consts.uniqueCoolOffStr}.`);
			}
