...
class ContentManager {
	...
	startPic(path, duration) { console.log("contentmanger: startPic");
		this.runningPicProc = cp.spawn('timeout', [duration + 's', './bin/eog', path, '-f']);
	}
	...
	downloadYtInfo(urlOrId) { console.log("contentmanger: downloadYtInfo");
		return new Promise(function(resolve, reject) {
			let infoProc = cp.spawn('./bin/youtube-dl', ['--no-playlist', '--get-title', '--get-duration', '--max-filesize', musicSizeLimit, urlOrId]);
			let rawData = '';
	...
	downloadYtVid(urlOrId, destination) { console.log("contentmanger: downloadYtVid");
		return new Promise(function(resolve, reject) {
			const dlProc = cp.spawn('./bin/youtube-dl', ['--no-playlist', urlOrId, '--max-filesize', musicSizeLimit, '-o', destination]);
