...
const { CancelError, DownloadTooLargeError, DownloadWrongTypeError, UniqueError, YTError } = require('./errors.js');
...
class ContentManager extends EventEmitter {
		...
		const p = new Promise(function(resolve, reject) {
			...
			if (that.ytIdIsUnique(ytId)) {
				that.downloadYtInfo(uplData.music.path).then((info) => {
					...
					debug.error(e);
					reject(new YTError(`I could not find the YouTube video requested (${uplData.music.ytId}). Is the URL correct?`));
				});
			
			} else {
				const when = consts.uniqueCoolOffStr.startsWith('Infinity') ? 'already' : `within the past ${consts.uniqueCoolOffStr}`;
				reject(new YTError(`I did not queue the YouTube video (${ytId}) you requested because it has been played ${when}.`));
			}
		});
	...
	cancelDownload(uid, contentId) {
		return this.ytDownloader.tryCancel(uid, contentId);
	}
	...
	downloadPic(url, destination) {
		return new Promise((resolve, reject) => {
			request.head(url, (err, res, body) => {
				...
				if (!res) {
					reject(new UnknownDownloadError(ContentType.pic, 'Could not get a response for the request.'));
				}

				const typeFound = res.headers['content-type'];

				if (typeFound.split('/')[0] !== 'image') {
					reject(new DownloadWrongTypeError(ContentType.pic, typeFound));
				}
				if (res.headers['content-length'] > opt.imageSizeLimit) {
					reject(new DownloadTooLargeError(ContentType.pic));
				}
