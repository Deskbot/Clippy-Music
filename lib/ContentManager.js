...
class ContentManager {
	...
	start() { console.log("contentmanger: start");
		...
		co(function*() {
			while (true) {
				...
				let musicProc = this.startMusic(contentData.musicPath, opt.timeout);
				...
				fs.appendFile(logFilePath, `${contentData.userId} played ${contentData.musicTitle} with ${picName} at ${currentTime}.\n`);
		...
		co(function*() {
			while (true) {
				...
				if (dlInfo.musicIsLocal) {
					...
					console.log('nmp', nmp);
					try {
						yield this.downloadYtVid(itemData.ytId, nmp);
	...
	startMusic(path, duration) { console.log("contentmanger: startMusic");
		return this.runningMusicProc = cp.spawn('timeout', [duration + 's', 'mpv', '-vo', 'xv', '-fs', '-quiet', '--af=drc=2:0.25', path]); //settings match those by music.get
	}
	...
	downloadYtInfo(urlOrId) { console.log("contentmanger: downloadYtInfo");
		return new Promise(function(resolve, reject) {
			let infoProc = cp.spawn('youtube-dl', ['--no-playlist', '--get-title', '--get-duration', '--max-filesize', musicSizeLimit, urlOrId]);
			let rawData = '';
			let rawError = '';
	...
	downloadYtVid(urlOrId, destination) { console.log("contentmanger: downloadYtVid");
		return new Promise(function(resolve, reject) {
			const dlProc = cp.spawn('youtube-dl', ['--no-playlist', urlOrId, '--max-filesize', musicSizeLimit, '-o', destination]);
			let rawError = '';
	...
	getBucketsForPublic() { console.log("contentmanger: getQueueForPublic");
		const usersToBuckets = this.playQueue.getBuckets();
		const userRecord = require('../main.js').userRecord; //I don't feel good doing it this way either

		return this.playQueue.getUsersByPosteriority().filter((userId) => {
			return {
				nickname: Html5Entities.encode(userRecord.getNickname(userId)),
				bucket: Html5Entities.encode(usersToBuckets[userId]),
			};
		});
	}
