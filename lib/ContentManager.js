...
const { CancelError, DownloadTooLargeError, DownloadWrongTypeError, UniqueError, UnknownDownloadError, YTError } = require('./errors.js');
	...
	add(uplData) {
		...
		const p = new Promise(function(resolve, reject) {
			...
			if (that.ytIdIsUnique(ytId)) {
				...
				reject(new UniqueError(ContentType.music));
			}
		});
		...
		return new Promise((resolve, reject) => {
			request.head(url, (err, res, body) => {
				...
				if (typeFound.split('/')[0] !== 'image') {
					reject(new DownloadWrongTypeError(ContentType.pic, 'image', typeFound));
				}
