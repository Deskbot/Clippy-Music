...
const WebSocketServer = require('../serv/WebSocketServer.js');
...
const streamYtOverThisDuration = 1200; //20 minutes
...
class ContentManager {
	...
	start() {
		...
		co(function*() {
			while (true) {
				...
				let musicProc = this.startMusic(contentData.musicPath, opt.timeout, contentData.startTime, contentData.endTime);

				musicProc.stdout.on('data', (buf) => {
					...
					if (buf.includes('(+)') || buf.includes('Audio') || buf.includes('Video')) {
						let picName;
						if (contentData.hasPic) this.startPic(contentData.picPath, opt.timeout);
					}
				});
				...
				WebSocketServer.broadcastQueue();
		...
		let musicHash, picHash; //memory efficiency
		...
		co(function*() {
			while (true) {
				...
				const itemData = dlInfo;//utils.clone(dlInfo);
				...
				let musicIsUnique = true;
				let picIsUnique = true;
				let ytIsUnique = true;
				...
				if (dlInfo.musicIsLocal) {
					...
					musicHash = utils.fileHash(itemData.musicPath).toString(); 
					if (musicIsUnique = this.hashIsUnique(musicHash)) { 
						itemData.musicHash = musicHash; 
					} else { 
					...
					ytIsUnique = this.ytIdIsUnique(itemData.ytId);
					...
					if (itemData.duration <= streamYtOverThisDuration) {
						let nmp = this.nextMusicPath();
						itemData.musicPath = nmp;

						try {
							let st = new Date().getTime();
							yield this.downloadYtVid(itemData.musicUrl, nmp);
							let et = new Date().getTime();
							let d = (et - st) / 1000;

							console.log('Yt vid length ' + itemData.duration + 's' + ', took ' + d + 's to download, vid secs per second ' + (itemData.duration / d));
						...
						//validate by music hash
						musicHash = utils.fileHash(nmp).toString();
						if (musicIsUnique = this.hashIsUnique(musicHash)) {
							itemData.musicHash = musicHash;
						} else {
							result.musicUniqueProblem = true;
						...
						itemData.musicPath = itemData.musicUrl;
						itemData.stream = true;
					}
				}
				...
				if (dlInfo.hasPic) {
					...
					picHash = utils.fileHash(itemData.picPath);
					if (picIsUnique = this.picHashIsUnique(picHash)) {
						itemData.picHash = picHash;
					} else {
				...
				if (musicIsUnique && picIsUnique && ytIsUnique) {
					this.addHash(musicHash);
					if (dlInfo.hasPic) this.addPicHash(picHash);
					if (!dlInfo.musicIsLocal) this.addYtId(itemData.ytId);
				}
		...
		return new Promise(function(resolve, reject) {
			if (!info.musicIsLocal) {
				...
				that.downloadYtInfo(info.musicUrl).then(function(data) {
					...
					console.error(e);
					reject('Failed to retreive data about the YouTube video requested: ' + JSON.stringify(info));
				});
	...
	deleteSuspended() {
		utils.deleteFileIfExistsSync(ContentManager.suspendedFilePath);
		utils.deleteFileIfExistsSync(ContentManager.logFilePath);
	}
}
