...
class ContentManager {
	constructor() { console.log("contentmanger: constructor");
		...
		if (success) {
			...
			this.dlQueue = obj.dlQueue;
			this.playQueue = new Queue(obj.playQueue);
			this.hashes = obj.hashes;
			this.picHashes = obj.picHashes;
			this.ytIds = obj.ytIds;
	...
	startPic(path, duration) { console.log("contentmanger: startPic");
		this.runningPicProc = cp.spawn('timeout', [duration + 's', __dirname + './bin/eog', path, '-f']);
	}
	...
	hashIsUnique(hash) { console.log("contentmanger: hashIsUnique");
		...
		console.log('All hashes', this.hashes);
		return !this.hashes[hash];
	}

	picHashIsUnique(hash) {
		return !this.picHashes[hash];
	}
	...
	downloadYtInfo(urlOrId) { console.log("contentmanger: downloadYtInfo");
		return new Promise(function(resolve, reject) {
			let infoProc = cp.spawn(__dirname + './bin/youtube-dl', ['--no-playlist', '--get-title', '--get-duration', '--max-filesize', musicSizeLimit, urlOrId]);
			let rawData = '';
			let rawError = '';
	...
	downloadYtVid(urlOrId, destination) { console.log("contentmanger: downloadYtVid");
		return new Promise(function(resolve, reject) {
			const dlProc = cp.spawn(__dirname + './bin/youtube-dl', ['--no-playlist', urlOrId, '--max-filesize', musicSizeLimit, '-o', destination]);
			let rawError = '';
	...
	store() { console.log("contentmanger: store");
		...
		console.log('new', JSON.stringify(storeObj));
