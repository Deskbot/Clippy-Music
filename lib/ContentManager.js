...
class ContentManager extends EventEmitter {
	...
	getBucketsForPublic() {
		...
		for (let i=0; i < userIds.length; i++) {
			...
			if (bucketTitles.length !== 0) {
				returnList.push({
					bucket: bucketTitles,
					nickname: UserRecordService.getNickname(userId),
					userId: userId,
				});
			}
		}
	...
	getCurrentlyPlaying() {
		if (this.currentlyPlaying) return {
			nickname: UserRecordService.getNickname(this.currentlyPlaying.userId),
	...
	logPlay(contentData) {
		const nickname = UserRecordService.getNickname(contentData.userId);
