...
class ContentManager extends EventEmitter {
	...
	musicHashIsUnique(hash) {
		...
		return !t || t + opt.musicUniqueCoolOff * 1000 < new Date().getTime();
	}
	...
	picHashIsUnique(hash) {
		...
		return !t || t + opt.imageUniqueCoolOff * 1000 < new Date().getTime();
	}
	...
	playNext() {
		...
		if (!this.musicHashIsUnique(contentData.music.hash)) {
