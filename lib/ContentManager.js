...
const UploadDlManager = require('./UploadDlManager.js');
...
class ContentManager extends EventEmitter {
	constructor(startState, idFactory) {
		super();
		...
		this.musicHashes = {};
		this.picHashes = {};
		this.uploadDlManager = new UploadDlManager();
	...
	tryQueue(itemData) {
		itemData.id = this.idFactory.new();
	...
	tryPrepMusic(itemData) {
		if (itemData.music.isUrl) {
			if (itemData.duration <= opt.streamYtOverDur) {
				...
				const downloadedProm = this.ytDlManager.new(itemData.id, itemData.userId, itemData.music.title, itemData.music.path, nmp);
