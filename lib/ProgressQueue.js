...
class ProgressQueue extends EventEmitter {
	...
	add(userId, title) {
		...
		const deleter = ProgressQueue.createDeleter(userId, newItem);
		const updater = ProgressQueue.createUpdater(newItem);
	...
	static createDeleter(userId, item) {
		return () => {
			this.delete(userId, item);
		};
	}

	static createUpdater(item) {
		return (percent) => {
