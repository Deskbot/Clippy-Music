...
class ProgressQueue extends EventEmitter {
	...
	add(userId, contentId, title) {
		...
		if (title) this.emitAdd(userId, newItem); // wait for all the data first
	}
	...
	setTitle(userId, contentId, title) {
		const item = this.findQueueItem(userId, contentId);
		item.title = title;

		if (!item.isSent) this.emitAdd(userId, item); // might have just gained all the data needed to show it to the user
