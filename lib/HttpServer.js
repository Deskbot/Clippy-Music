	...
	start: function(environmentData) {
		...
		return new Promise((resolve, reject) => {
			...
			app.post('/api/content/upload', recordUserMiddleware, (req, res) => {
				...
				new Promise(function(resolve, reject) {
						...
						uploadInfo.deferred.promise.then(
							...
							(result) => {
								...
								if (result.musicUniqueProblem) {
									probs.push(`I was unable to play the music you requested because it has been played in the past ${utils.secToTimeStr(opt.uniquenessCoolOff)}.`);
								}
								if (result.picUniqueProblem) {
									probs.push(`I didn't queue the picture you requested because it has been shown in the past ${utils.secToTimeStr(opt.uniquenessCoolOff)}.`);
								}

								main.sendError(req.ip, 'upload', probs);
							}
						);
					...
					//this section regards the result of trying to add the data to the play queue
						...
						res.status(500).end(messages.join(' & '));
					});
			...
			app.post('/api/content/remove', (req, res) => {
				if (!environmentData.contentManager.remove(socToUserId(soc), req.body.contentId)) {
			...
			app.post('/api/ban/add', (req, res) => {
				if (environmentData.adminPassword !== null && req.body.password === environmentData.adminPassword) {
					if (environmentData.userRecord.isUser(req.body.id)) {
						environmentData.userRecord.addBan(req.body.id);
						environmentData.contentManager.purgeUser(req.body.id);
			...
			app.post('/api/ban/remove', (req, res) => {
				if (environmentData.adminPassword !== null && req.body.password === environmentData.adminPassword) {
					if (environmentData.userRecord.isBanned(req.body.id)) {
			...
			app.post('/api/nickname/set', recordUserMiddleware, (req, res) => {
				...
				environmentData.userRecord.setNickname(req.ip, Html5Entities.encode(req.body.nickname.substr(0, opt.nicknameSizeLimit)));
